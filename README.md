...
Система потоковой обработки рыночных событий для обнаружения ценовых импульсов в реальном времени.
Фильтры (impulse) создаются через REST API → уходят в Kafka → Flink применяет их к потоку трейдов → алерты публикуются в Kafka.

Решает проблему ручного мониторинга рынка: трейдер задаёт условие (направление, процент движения, временное окно) и получает алерт при его срабатывании. В перспективе система интегрируется в торговый терминал как встраиваемый модуль алертинга.
